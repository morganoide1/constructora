    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gestión de expensas por propiedad</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 text-gray-500 font-medium">Propiedad</th>
                <th className="text-left py-3 text-gray-500 font-medium">Período</th>
                <th className="text-left py-3 text-gray-500 font-medium">Vencimiento</th>
                ...
                <th className="text-right py-3 text-gray-500 font-medium">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {expensas.map(exp => (
                <tr key={exp._id} className="border-b border-gray-100 hover:bg-green-50">
                  <td className="py-3 text-gray-800">{exp.propiedad?.nombre || exp.propiedad?.codigo}</td>
                  <td className="py-3 text-gray-800">{meses[exp.periodo.mes - 1]} {exp.periodo.año}</td>
                  <td className="py-3 text-gray-500">{exp.fechaVencimiento ? new Date(exp.fechaVencimiento).toLocaleDateString() : '-'}</td>
                  <td className="py-3 text-right font-medium text-amber-600">{fmt(exp.montoTotal, exp.moneda)}</td>
                  <td className="py-3 text-center">
                    <span className={`px-2 py-1 rounded-full text-xs ${exp.estado === 'pagada' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                      {exp.estado}
                    </span>
                  </td>
                  <td className="py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {exp.estado === 'pendiente' && (
                        <button onClick={() => handlePagar(exp._id)} className="p-2 hover:bg-green-100 rounded-lg" title="Marcar pagada">
                          <Check className="w-4 h-4 text-emerald-600" />
                        </button>
                      )}
                      <button onClick={() => handleDelete(exp._id)} className="p-2 hover:bg-gray-100 rounded-lg">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-2">Monto Total</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-2">Ordinarias</label>
                  <input type="number" value={form.ordinarias} onChange={(e) => setForm({...form, ordinarias: e.target.value})} className="input-field" />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-2">Extraordinarias</label>
