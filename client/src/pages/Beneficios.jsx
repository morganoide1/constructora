...
function Beneficios() {
  ...
  const categoriaColors = {
    descuento: 'bg-emerald-500/20 text-emerald-400',
    servicio: 'bg-blue-500/20 text-blue-400',
    experiencia: 'bg-purple-500/20 text-purple-400',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-white">Beneficios Cliente Wave</h1>
          <p className="text-white/60 mt-1">Gestiona los beneficios exclusivos para clientes</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {beneficios.map((b) => (
          <div key={b._id} className={`card ${!b.activo ? 'opacity-50' : ''}`}>
            {b.imagen && (
              <img src={b.imagen} alt={b.titulo} className="w-full h-40 object-cover rounded-lg mb-4" />
            )}
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex gap-1">
                <button onClick={() => handleEdit(b)} className="p-1 hover:bg-white/10 rounded"><Edit className="w-4 h-4 text-white/60" /></button>
                <button onClick={() => handleDelete(b._id)} className="p-1 hover:bg-white/10 rounded"><Trash2 className="w-4 h-4 text-rose-400" /></button>
              </div>
            </div>
            <h3 className="text-lg font-semibold text-white mb-2">{b.titulo}</h3>
            <p className="text-white/60 text-sm mb-3">{b.descripcion}</p>
            {b.link && (
              <a href={b.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-sm flex items-center gap-1 hover:underline">
      ...
      {beneficios.length === 0 && (
        <div className="card text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">No hay beneficios</h3>
          <p className="text-white/60">Crea el primer beneficio para tus clientes</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-md">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-white">{editing ? 'Editar Beneficio' : 'Nuevo Beneficio'}</h3>
              <button onClick={closeModal} className="p-2 hover:bg-white/10 rounded-lg"><X className="w-5 h-5 text-white/60" /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-white/70 mb-2">URL Imagen</label>
                <input type="url" value={form.imagen} onChange={(e) => setForm({...form, imagen: e.target.value})} className="input-field" placeholder="https://..." />
              </div>
              <div>
                <label className="block text-sm text-white/70 mb-2">Link (opcional)</label>
